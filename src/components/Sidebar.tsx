...
export default function Sidebar({ 
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed top-16 left-0 h-[calc(100vh-4rem)] w-80 z-50 transform transition-transform duration-300 ease-in-out
      ...
      `}>
          ...
          <nav className="space-y-2">
            {menuItems.map((item) => (
              <button
                ...
                className={`
                  w-full flex items-center space-x-3 p-3 lg:p-4 rounded-xl transition-all duration-200 text-left group
                  ${currentPage === item.id
                    ? isDark
        ...
        {/* Generation Tracking */}
        <div className={`p-4 lg:p-6 border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} flex-shrink-0`}>
          <div className="space-y-4">
            ...
            {/* Upgrade Button */}
            <button className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 text-white py-2 lg:py-3 px-3 lg:px-4 rounded-xl font-semibold hover:from-yellow-600 hover:to-orange-600 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center space-x-2 group">
            ...
            {/* Status Message */}
            {generatedCount >= maxGenerations ? (
              <div className={`p-2 lg:p-3 rounded-lg ${isDark ? 'bg-red-900/20 border border-red-800' : 'bg-red-50 border border-red-200'}`}>
              ...
              <div className={`p-2 lg:p-3 rounded-lg ${isDark ? 'bg-yellow-900/20 border border-yellow-800' : 'bg-yellow-50 border border-yellow-200'}`}>
              ...
              <div className={`p-2 lg:p-3 rounded-lg ${isDark ? 'bg-green-900/20 border border-green-800' : 'bg-green-50 border border-green-200'}`}>
